...
<div id="respond" name="respond">
	...
	<h3><?php comment_form_title( __('Deja un comentario', TDOMAIN), __('Deja un comentario a %s', TDOMAIN) ); ?></h3>
	...
	<?php else : ?>
		...
		<label for="author"><small><?php _e('Nombre', TDOMAIN);?> <?php if ($req)  _e("(obligatorio )",TDOMAIN); ?></small></label></p>

		<p><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> />
		<label for="email"><small><?php _e('E-Mail (no será publicado)', TDOMAIN);?> <?php if ($req) _e('(requerido)', TDOMAIN); ?></small></label></p>

		<p><input type="text" name="url" id="url" value="<?php echo $comment_author_url; ?>" size="22" tabindex="3" />
		<label for="url"><small><?php _e('Sitio web', TDOMAIN);?></small></label></p>

		<?php endif; ?>

		<?php if(get_option('show_xhtml')):?>
			<p><small><strong>XHTML:</strong> <?php _e('Tu puedes usar éstas tags:', TDOMAIN);?> <code><?php echo allowed_tags(); ?></code></small></p>
		<?php endif;?>

		<p><textarea name="comment" id="comment" cols="100%" rows="10" tabindex="4"></textarea></p>

		<p><input name="submit" type="submit" id="submit" tabindex="5" value="<?php _e('Enviar comentario', TDOMAIN);?>" />
